...
def load_ld_map(species_name):
    ...
    ld_directory = '%slinkage_disequilibria_per_gene/' % (parse_midas_data.data_directory)
    ...
    for line in file:
        ...
        gene_name = items[3].strip()

        if (clade_type, variant_type) not in ld_map:

            ld_map[(clade_type, variant_type)] = {}
        ...
        #ld_map[(clade_type, variant_type)][gene_name] = (distances, intragene_rsquared_numerators, intragene_rsquared_denominators, intragene_counts, intergene_distances, intergene_rsquared_numerators, intergene_rsquared_denominators, intergene_counts, control_numerator, control_denominator, control_count)
        ld_map[(clade_type, variant_type)][gene_name] = (distances, intragene_rsquared_numerators, intragene_rsquared_denominators, intragene_counts)
...
def calculate_ld_dict(good_species_list, subject_sample_map, bin_width_exponent=0.1, variant_types = ['4D','1D'], debug=False):
    ...
    for species_name in good_species_list:
        ...
        for variant_type in variant_types:
            ...
            #for kegg_id in kegg_ids_filtered:

            ld_dict[variant_type] = {}

            print(ld_map[('largest_clade',variant_type)].keys())
...
if __name__=='__main__':
    ...
    record_strs = [", ".join(['Species', 'CladeType', 'VariantType', 'KEGG ID']+distance_strs)]
    ...
    for species_name in good_species_list:
        ...
        sys.stderr.write("Loading KEGG annotations...\n")
        ...
        kegg_ids_filtered = [kegg_ids[k][0][0] for k in kegg_ids.keys() if len(kegg_ids[k][0][0])>0]
        #kegg_ids['435590.9.peg.1961'][0][0]

        print(kegg_ids_filtered)
        ...
        for clade_type in clade_types:
            for variant_type in variant_types:
                ...
                #neighboring_gene_rsquared_numerators[(clade_type,variant_type)] = {}
                #neighboring_gene_rsquared_denominators[ (clade_type,variant_type)] = {}
                #neighboring_gene_counts[(clade_type,variant_type)] = {}

                #total_control_rsquared_numerators[(clade_type,variant_type)] = {}
                #total_control_rsquared_denominators[(clade_type,variant_type)] = {}
                #total_control_counts[(clade_type,variant_type)] = {}

                #total_control_genes[(clade_type,variant_type)] = {}

                #for gene_name in gene_size_dict.keys():
                for kegg_id in kegg_ids_filtered:

                    binned_rsquared_numerators[(clade_type,variant_type)][kegg_id] = numpy.zeros_like(distance_bin_locations)
                    binned_rsquared_denominators[(clade_type,variant_type)][kegg_id] = numpy.zeros_like(distance_bin_locations)
                    binned_counts[(clade_type,variant_type)][kegg_id] = numpy.zeros_like(distance_bin_locations)

                    #neighboring_gene_rsquared_numerators[(clade_type,variant_type)][kegg_id] = numpy.zeros_like(neighbor_distances)*1.0
                    #neighboring_gene_rsquared_denominators[ (clade_type,variant_type)][kegg_id] = numpy.zeros_like(neighbor_distances)*1.0
                    #neighboring_gene_counts[(clade_type,variant_type)][kegg_id] = numpy.zeros_like(neighbor_distances)*1.0

                    #total_control_rsquared_numerators[(clade_type,variant_type)][kegg_id] = 0
                    #total_control_rsquared_denominators[(clade_type,variant_type)][kegg_id] = 0
                    #total_control_counts[(clade_type,variant_type)][kegg_id] = 0

                    #total_control_genes[(clade_type,variant_type)][kegg_id] = []
            ...
            for clade_type in clade_types:
                ...
                for variant_type in variant_types:
                    ...
                    for gene_name in allele_counts_map.keys():
                        ...
                        kegg_id_gene = kegg_ids[gene_name][0][0]

                        if len(kegg_id_gene) == 0:
                            continue
                        ...
                        #rsquared_numerators, rsquared_denominators = diversity_utils.calculate_conditional_unbiased_sigmasquared(allele_counts, allele_counts, condition_on_freq, low, high)
                        ...
                        #        #chunk_rsquared_numerators, chunk_rsquared_denominators = diversity_utils.calculate_conditional_unbiased_sigmasquared(allele_counts, neighboring_allele_counts, condition_on_freq, low, high)
                        ...
                        #    #control_gene_rsquared_numerators, control_gene_rsquared_denominators = diversity_utils.calculate_conditional_unbiased_sigmasquared(allele_counts, control_allele_counts, condition_on_freq, low, high)
                        ...
                        for i in xrange(0,len(bin_idxs)):
                            ...
                            binned_counts[(clade_type,variant_type)][kegg_id_gene][bin_idxs[i]] += 1
                            binned_rsquared_numerators[(clade_type,variant_type)][kegg_id_gene][bin_idxs[i]] += rsquared_numerators[i]
                            binned_rsquared_denominators[(clade_type,variant_type)][kegg_id_gene][bin_idxs[i]] += rsquared_denominators[i]
                        ...
                        #for i in xrange(0,len(neighbor_distances)):
                            ...
                            #neighboring_gene_counts[(clade_type,variant_type)][kegg_id_gene][i] += good_idxs.sum()

                            #neighboring_gene_rsquared_numerators[(clade_type,variant_type)][kegg_id_gene][i] += neighbor_rsquared_numeratorss[i][good_idxs].sum()

                            #neighboring_gene_rsquared_denominators[(clade_type,variant_type)][kegg_id_gene][i] += neighbor_rsquared_denominatorss[i][good_idxs].sum()


                        #total_control_counts[(clade_type,variant_type)][gene_name] += (control_rsquared_denominators>1e-09).sum()
                        #total_control_rsquared_numerators[(clade_type,variant_type)][gene_name] += control_rsquared_numerators[control_rsquared_denominators>1e-09].sum()
                        #total_control_rsquared_denominators[(clade_type,variant_type)][gene_name] += control_rsquared_denominators[control_rsquared_denominators>1e-09].sum()

                        #total_control_genes[(clade_type,variant_type)][gene_name].append(gene_name)
        ...
        for clade_type in clade_types:
            for variant_type in variant_types:
                ...
                for kegg_id in kegg_ids_filtered:
                    ...
                    binned_rsquareds = binned_rsquared_numerators[(clade_type,variant_type)][kegg_id]*1.0/(binned_rsquared_denominators[(clade_type,variant_type)][kegg_id] + (binned_rsquared_denominators[(clade_type,variant_type)][kegg_id] == 0))

                    #control_rsquareds = total_control_rsquared_numerators[(clade_type,variant_type)][gene_name]*1.0/(total_control_rsquared_denominators[(clade_type,variant_type)][gene_name]+(total_control_rsquared_denominators[(clade_type,variant_type)][gene_name]==0))

                    rsquared_strs = ["%g:%g:%d" % (rsquared_numerator, rsquared_denominator, count) for rsquared_numerator, rsquared_denominator, count in zip(binned_rsquared_numerators[(clade_type,variant_type)][kegg_id], binned_rsquared_denominators[(clade_type,variant_type)][kegg_id], binned_counts[(clade_type,variant_type)][kegg_id])[1:-1]]

                    #gene_rsquared_strs = ["%g:%g:%d" % (rsquared_numerator, rsquared_denominator, count) for rsquared_numerator, rsquared_denominator, count in zip(neighboring_gene_rsquared_numerators[(clade_type,variant_type)][kegg_id], neighboring_gene_rsquared_denominators[(clade_type,variant_type)][kegg_id], neighboring_gene_counts[(clade_type,variant_type)][kegg_id])]

                    #control_rsquared_str = "%g:%g:%d" % (total_control_rsquared_numerators[(clade_type,variant_type)][gene_name], total_control_rsquared_denominators[(clade_type,variant_type)][gene_name], total_control_counts[(clade_type,variant_type)][gene_name])

                    #pi_str = str(pi)

                    #record_str = ", ".join([species_name, clade_type, variant_type, pi_str]+rsquared_strs+gene_rsquared_strs+[control_rsquared_str])
                    #record_str = ", ".join([species_name, clade_type, variant_type, gene_name]+rsquared_strs+gene_rsquared_strs+[control_rsquared_str])
                    record_str = ", ".join([species_name, clade_type, variant_type, kegg_id]+rsquared_strs)
