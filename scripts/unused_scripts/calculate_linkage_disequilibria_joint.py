...
if __name__=='__main__':
    ...
    #condition_on_freq = args.condition_on_freq
    #low = args.low
    #high = args.high

    # minimum number of sites that have to occur in 4D and 1D sites
    n_sites_min = 4

    ld_non_syn_directory = '%slinkage_disequilibria_non_syn/' % (parse_midas_data.data_directory)
    ...
    # the binning scheme depends on the data..
    ...
    record_per_gene_strs = [", ".join(['Species', 'CladeType', 'VariantType', 'GeneName', 'GeneSize', 'MinimumDistance', 'Distances', 'RsquaredNumerators', 'RsquaredDenominators'])]

    os.system('mkdir -p %s' % ld_non_syn_directory)
            ...
            for clade_type in clade_types:
                ...
                for variant_type in variant_types:
                    ...
                    for gene_name in allele_counts_map.keys():
                        ...
                        rsquared_numerators, rsquared_denominators, rsquared_ns = diversity_utils.calculate_unbiased_sigmasquared(allele_counts, allele_counts)
                        #print(rsquared_denominators,allele_counts)
                        #rsquared_numerators, rsquared_denominators = diversity_utils.calculate_unbiased_sigmasquared(allele_counts, allele_counts)
                        ...
                        for neighbor_distance_idx in xrange(0,len(neighbor_distances)):
                            ...
                            for neighboring_gene_name in neighboring_genes:
                                ...
                                chunk_rsquared_numerators, chunk_rsquared_denominators, chunk_rsquared_ns = diversity_utils.calculate_unbiased_sigmasquared(allele_counts, neighboring_allele_counts)
                        ...
                        for control_idx in xrange(0,10):
                            ...
                            control_gene_rsquared_numerators, control_gene_rsquared_denominators, control_gene_rsquared_ns = diversity_utils.calculate_unbiased_sigmasquared(allele_counts, control_allele_counts)
                        ...
                        #print(allele_counts.shape)
                        #print(allele_counts[0,:,:].shape)
                        #print(allele_counts[0,:,:])


                        #get_allele_pair_counts = []
                        ...
                        # per-gene LD dict
                        #print(allele_counts)
                        #print(allele_counts[0,0,:])

                        #allele_counts_filter = allele_counts[desired_idxs,:,:]
                        #print(distances)
                        #print(desired_idxs)
                        #print(allele_counts_filter.shape)

                        #allele_counts_filter = allele_counts_filter[rsquared_denominators>1e-09,:,:]

                        # not worrying about allele counts right now,
