...
min_ld_sample_size = 20

allowed_variant_types = set(['1D','4D'])

def load_ld_map(species_name, f0, ld_moment, same_f0_range=False):
...
def calculate_ld_dict(good_species_list, subject_sample_map, f0, ld_moment, same_f0_range=False,  bin_width_exponent=0.1, variant_types = ['4D','1D'], debug=False):
    ...
    for species_name in good_species_list:
        ...
        # Load precomputed LD
        ld_map = load_ld_map(species_name, f0, ld_moment, same_f0_range=same_f0_range)
        ld_dict = {}
...
def get_f0_range(n_range=40, largest_clade=True):
...
def get_f0_range_all_species(n_range=40):

    f0_range = numpy.logspace(numpy.log10(1/min_ld_sample_size), numpy.log10(0.1), num=n_range, endpoint=True, base=10.0)

    return f0_range


def get_f0_range_species(specis_name, n_range=40, largest_clade=True):
    ...
    #sys.stderr.write("Max clade size: %d\n" % len(largest_clade_samples))
    # f0_min = 3/N
    f0_range = numpy.logspace(numpy.log10(1/len(largest_clade_set)), numpy.log10(0.1), num=n_range, endpoint=True, base=10.0)
...
def get_no_nan_inf_idxs(distances, rsquared_numerators, rsquared_denominators):


    no_nan_idxs = []
    for distance_i_idx, distance_i in enumerate(distances):


        if (numpy.isnan(rsquared_numerators[distance_i_idx]) == True) or (numpy.isnan(rsquared_denominators[distance_i_idx]) == True) or (numpy.isinf(rsquared_numerators[distance_i_idx]) == True) or (numpy.isinf(rsquared_denominators[distance_i_idx]) == True):

            no_nan_idxs.append(False)

        else:
            no_nan_idxs.append(True)

    no_nan_idxs = numpy.asarray(no_nan_idxs)

    return no_nan_idxs



#def make_ld_ratio_dict_all_species():
...
if __name__=='__main__':
    ...
    parser.add_argument("--same_f0_range", help="Use the same f0 range for all species", action="store_true")
    ...
    same_f0_range = args.same_f0_range
    ...
    #f0_range =  numpy.logspace(numpy.log10(1/len(largest_clade_samples)), numpy.log10(0.2), num=20, base=10, endpoint=True)
    ...
    for species_name in good_species_list:
        ...
        if same_f0_range:
            f0_range = get_f0_range_all_species()

        else:
            f0_range = get_f0_range_species(species_name)
        ...
        for f0 in f0_range:
                ...
                for clade_type in clade_types:
                    ...
                    for variant_type in variant_types:
                        ...
                        for gene_name in allele_counts_map.keys():
                            ...
                            genotypes_1, passed_sites_1 = diversity_utils.calculate_consensus_genotypes(allele_counts)
                            genotypes_2, passed_sites_2 = diversity_utils.calculate_consensus_genotypes(allele_counts)
                            joint_passed_sites = (passed_sites_1)[None,:,:]*(passed_sites_2)[:,None,:]
                            # sites x sites x samples matrix
                            # allele counts
                            ns = joint_passed_sites.sum(axis=2)
                            if numpy.any(ns*f0<=2.0) == True:
                                continue


                            #rsquared_numerators, rsquared_denominators = diversity_utils.calculate_unbiased_sigmasquared(allele_counts, allele_counts)
                            #rsquared_numerators, rsquared_denominators = diversity_utils.calculate_conditional_unbiased_sigmasquared(allele_counts, allele_counts, low_freq=f0)
                            ...
                            #if (numpy.isnan(rsquared_numerators).any()) or (numpy.isnan(rsquared_denominators).any()):
                            #    continue
                            #if (numpy.isinf(rsquared_numerators).any()) or (numpy.isinf(rsquared_denominators).any()):
                            #    continue


                            # if either the numerator or denominator in a given distance bin are nan or inf
                            # replace those values
                            #rsquared_numerators[numpy.logical_or(rsquared_numerators == numpy.inf, rsquared_denominators == numpy.inf)] = 0
                            #rsquared_denominators[numpy.logical_or(rsquared_numerators == numpy.inf, rsquared_denominators == numpy.inf)] = 0

                            #rsquared_numerators[numpy.logical_or(rsquared_numerators == numpy.nan, rsquared_denominators == numpy.nan)] = 0
                            #rsquared_denominators[numpy.logical_or(rsquared_numerators == numpy.nan, rsquared_denominators == numpy.nan)] = 0
                            ...
                            for neighbor_distance_idx in xrange(0,len(neighbor_distances)):
                                ...
                                for neighboring_gene_name in neighboring_genes:
                                    ...
                                    #chunk_rsquared_numerators, chunk_rsquared_denominators = diversity_utils.calculate_conditional_unbiased_sigmasquared(allele_counts, neighboring_allele_counts, low_freq=f0)
                                    ...
                                    chunk_rsquared_numerators[numpy.logical_or(chunk_rsquared_numerators == numpy.inf, chunk_rsquared_denominators == numpy.inf)] = 0
                                    #chunk_rsquared_denominators[numpy.logical_or(chunk_rsquared_numerators == numpy.inf, chunk_rsquared_denominators == numpy.inf)] = 0

                                    #chunk_rsquared_numerators[numpy.logical_or(chunk_rsquared_numerators == numpy.nan, chunk_rsquared_denominators == numpy.nan)] = 0
                                    #chunk_rsquared_denominators[numpy.logical_or(chunk_rsquared_numerators == numpy.nan, chunk_rsquared_denominators == numpy.nan)] = 0
                            ...
                            for control_idx in xrange(0,10):
                                ...
                                #control_gene_rsquared_numerators, control_gene_rsquared_denominators = diversity_utils.calculate_conditional_unbiased_sigmasquared(allele_counts, control_allele_counts, low_freq=f0)
                                ...
                                #control_gene_rsquared_numerators[numpy.logical_or(control_gene_rsquared_numerators == numpy.inf, control_gene_rsquared_denominators == numpy.inf)] = 0
                                #control_gene_rsquared_denominators[numpy.logical_or(control_gene_rsquared_numerators == numpy.inf, control_gene_rsquared_denominators == numpy.inf)] = 0

                                #control_gene_rsquared_numerators[numpy.logical_or(control_gene_rsquared_numerators == numpy.nan, control_gene_rsquared_denominators == numpy.nan)] = 0
                                #control_gene_rsquared_denominators[numpy.logical_or(control_gene_rsquared_numerators == numpy.nan, control_gene_rsquared_denominators == numpy.nan)] = 0
                            ...
                            no_nan_idxs = get_no_nan_inf_idxs(distances, rsquared_numerators, rsquared_denominators)


                            if len(no_nan_idxs) == 0:
                                continue



                            distances = distances[no_nan_idxs]
                            rsquared_numerators = rsquared_numerators[no_nan_idxs]
                            rsquared_denominators = rsquared_denominators[no_nan_idxs]
                            ...
                            for i in xrange(0,len(neighbor_distances)):
                                ...
                                neighbor_rsquared_denominatorss_i = neighbor_rsquared_denominatorss[i]
                                neighbor_rsquared_numeratorss_i = neighbor_rsquared_numeratorss[i]


                                neighbor_no_nan_idxs = get_no_nan_inf_idxs(list(range(len(neighbor_rsquared_numeratorss_i))), neighbor_rsquared_numeratorss_i, neighbor_rsquared_denominatorss_i)

                                if len(neighbor_no_nan_idxs) == 0:
                                    continue

                                neighbor_rsquared_numeratorss_i = neighbor_rsquared_numeratorss_i[neighbor_no_nan_idxs]
                                neighbor_rsquared_denominatorss_i = neighbor_rsquared_denominatorss_i[neighbor_no_nan_idxs]

                                good_idxs = (neighbor_rsquared_denominatorss_i>1e-09)
                                neighboring_gene_counts[(clade_type,variant_type)][i] += good_idxs.sum()

                                neighboring_gene_rsquared_numerators[ (clade_type,variant_type)][i] += neighbor_rsquared_numeratorss_i[good_idxs].sum()
                                neighboring_gene_rsquared_denominators[ (clade_type,variant_type)][i] += neighbor_rsquared_denominatorss_i[good_idxs].sum()
                            ...
                            total_control_no_nan_idxs = get_no_nan_inf_idxs(list(range(len(control_rsquared_numerators))), control_rsquared_numerators, control_rsquared_denominators)

                            if len(total_control_no_nan_idxs) == 0:
                                continue

                            control_rsquared_numerators = control_rsquared_numerators[total_control_no_nan_idxs]
                            control_rsquared_denominators = control_rsquared_denominators[total_control_no_nan_idxs]
            ...
            ld_map = load_ld_map(good_species_list[0], f0_range[0], ld_moment, same_f0_range=same_f0_range)
