...
#good_species_list = ['Akkermansia_muciniphila_55290']

#good_species_list = ['Bacteroides_vulgatus_57955', 'Akkermansia_muciniphila_55290']

bin_width_exponents = [0.3, 0.5, 0.7, 0.9, 1.1]
...
for bin_width_exponent_idx, bin_width_exponent in enumerate(bin_width_exponents):
    ...
    ld_dict_all_species = calculate_ld_dict(good_species_list, subject_sample_map, bin_width_exponent=bin_width_exponent)
    ...
    for species_name in ld_dict_all_species.keys():
        ...
        ld_filter_distances = numpy.intersect1d(ld_1D_distances, ld_4D_distances)
        ...
        ld_1D_filter_rsquareds = ld_dict['1D']['rsquareds'][ld_1D_filter_idx]
        ld_4D_filter_rsquareds = ld_dict['4D']['rsquareds'][ld_4D_filter_idx]
    ...
    fig.savefig("%s%s_%d.png" % (config.analysis_directory, 'ld_ratio_all_species', bin_width_exponent_idx), format='png', bbox_inches = "tight", pad_inches = 0.4, dpi = 600)
    plt.close()
...
for species_name in ld_dict_all_species.keys():
    ...
    print(len(ld_1D_filter_rsquareds), len(ld_4D_filter_rsquareds))

    #no_zeros_idx = (rsquareds_4D > 0) & (rsquareds_1D > 0)
    continue
