...
for line in snp_file:
    ...
    for i in xrange(0,len(alts)):
        ...
        if site not in site_map[i][variant_type]:
            ...
            site_map[i][variant_type][site] = [0,0.0,'',0,'','']
        #
        site_map[i][variant_type][site][0] += 1
        site_map[i][variant_type][site][1] += population_freq # weight of polarization reversals
        site_map[i][variant_type][site][2] = chromosome
        site_map[i][variant_type][site][3] = location
        site_map[i][variant_type][site][4] = gene_name
        site_map[i][variant_type][site][5] = variant_type
...
output_file = bz2.BZ2File("%ssnps/%s/within_sample_sfs_labelled.txt.bz2" % (parse_midas_data.data_directory, species_name),"w")

#output_file.write("\t".join(["SampleID", "variant_type", "D,A,count,reverse_count", "..."]))
output_file.write("\t".join(["SampleID", "variant_type", "D,A,count,reverse_count,chromosome,location,gene_name,variant_type", "..."]))
...
for sample_idx in xrange(0,len(samples)):
    ...
    for variant_type in allowed_variant_type_list:
        ...
        output_file.write("\t".join([sample, variant_type]+["%d,%d,%d,%g,%s,%s,%s,%s" % (site[0],site[1],site_map[sample_idx][variant_type][site][0],site_map[sample_idx][variant_type][site][1], site_map[sample_idx][variant_type][site][2], site_map[sample_idx][variant_type][site][3], site_map[sample_idx][variant_type][site][4], site_map[sample_idx][variant_type][site][5]) for site in sorted(site_map[sample_idx][variant_type].keys())]))
