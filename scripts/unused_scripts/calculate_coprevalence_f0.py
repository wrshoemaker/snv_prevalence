                        ...
                        for gene_name in allele_counts_map.keys():
                            ...
                            for neighbor_distance_idx in xrange(0,len(neighbor_distances)):
                                ...
                                for neighboring_gene_name in neighboring_genes:
                                    ...
                                    chunk_rsquared_numerators, chunk_rsquared_denominators = diversity_utils.calculate_unbiased_sigma_k_prevalence(allele_counts, neighboring_allele_counts, f0, k=ld_moment)
                            ...
                            #total_control_counts[(clade_type,variant_type)][f0] += (control_rsquared_denominators>1e-09).sum()
                            #total_control_rsquared_numerators[(clade_type,variant_type)][f0] += control_rsquared_numerators[control_rsquared_denominators>1e-09].sum()
                            #total_control_rsquared_denominators[(clade_type,variant_type)][f0] += control_rsquared_denominators[control_rsquared_denominators>1e-09].sum()
                            #print(((control_rsquared_denominators>1e-09) & ( ~numpy.isnan(control_rsquared_denominators)) & (numpy.isfinite(control_rsquared_denominators)) ).sum())
                            #total_control_counts[(clade_type,variant_type)][f0] += ((control_rsquared_denominators>1e-09) & ( ~numpy.isnan(control_rsquared_denominators)) & (numpy.isfinite(control_rsquared_denominators)) ).sum()
                            #total_control_rsquared_numerators[(clade_type,variant_type)][f0] += control_rsquared_numerators[(control_rsquared_denominators>1e-09) & ( ~numpy.isnan(control_rsquared_denominators)) & (numpy.isfinite(control_rsquared_denominators)) ].sum()
