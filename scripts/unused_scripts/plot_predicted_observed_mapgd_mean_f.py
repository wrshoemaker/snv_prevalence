...
for column_idx, column in enumerate(nested_species_list):
    ...
    for row_idx, row in enumerate(column):
        ...
        predicted_prevalence = prevalence_dict[row]['4D']['predicted_f_mean']

        predicted_prevalence = numpy.asarray(predicted_prevalence)

        observed_prevalence = prevalence_dict[row]['all']['4D']['f_mean']
...
# , dpi = 600
fig.savefig("%spredicted_observed_f_mean_mapgd.png" % (config.analysis_directory), format='png', bbox_inches = "tight", pad_inches = 0.4)
plt.close()
