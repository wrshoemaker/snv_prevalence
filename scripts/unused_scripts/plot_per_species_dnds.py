...
divergence_axis = fig.add_subplot(gs[0:2, :])
percentile_axis = fig.add_subplot(gs[2:, 0])
kegg_axis = fig.add_subplot(gs[2:, 1])


divergence_axis.text(-0.1, 1.0, sub_plot_labels[0], fontsize=16, fontweight='bold', ha='center', va='center', transform=divergence_axis.transAxes)
percentile_axis.text(-0.2, 1.0, sub_plot_labels[1], fontsize=16, fontweight='bold', ha='center', va='center', transform=percentile_axis.transAxes)
kegg_axis.text(-0.1, 1.0, sub_plot_labels[2], fontsize=16, fontweight='bold', ha='center', va='center', transform=kegg_axis.transAxes)
...
line.set_dashes((1,1))
divergence_axis.loglog(theory_ds, theory_dNdSs,'k--',linewidth=3,label='Negative selection model')
...
for label in labels:
	if 'metabolism' in label:
		label = label.replace('metabolism', 'metab.')
	if 'biosynthesis' in label:
		label = label.replace('biosynthesis', 'biosyn.')
	if 'pathway' in label:
		label = label.replace('pathway', 'path.')

	if 'interconversions' in label:
		label = label.replace('interconversions', 'inter.')

	if 'via hydroxylation' in label:
		label = label.replace('via hydroxylation', '')

	if 'Amino sugar' in label:
		label = label.replace('Amino sugar', 'Amino')

	if '(CO2 fixation)' in label:
		label = label.replace('(CO2 fixation)', '')

	if 'degradation' in label:
		label = label.replace('degradation', 'degrad.')

	if 'Phenylalanine, tyrosine and tryptophan' in label:
		label = label.replace('Phenylalanine, tyrosine and tryptophan', 'Tryptophan')


	if 'and other terpenoid-quinone' in label:
		label = label.replace('and other terpenoid-quinone', '')
...
kegg_axis.set_yticks(numpy.arange(len(labels_clean)))
kegg_axis.set_yticklabels(labels_clean, rotation=30)
kegg_axis.yaxis.set_tick_params(labelsize=9.5)
...
ax_taylors = inset_axes(kegg_axis, width="100%", height="100%", loc='lower right', bbox_to_anchor=(0.68,0.07,0.3,0.25), bbox_transform=kegg_axis.transAxes)
...
y_null_range = 10 ** (2*x_log10_fit_range + intercept)

ax_taylors.plot(10**x_log10_fit_range, y_fit_range, c='k', lw=3, linestyle='--', label='OLS fit', zorder=2)
ax_taylors.plot(10**x_log10_fit_range, y_null_range, c='k', lw=3, linestyle=':', label=r'$\mathrm{CV}=1$', zorder=2)
...
fig.subplots_adjust(hspace=0.4, wspace=0.33) #hspace=0.3, wspace=0.5
