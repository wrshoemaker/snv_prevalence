...
divergences =  numpy.logspace(numpy.log10(2e-06), numpy.log10(2e-04), num=5, base=10, endpoint=True)

clade_types = ['all','largest_clade']
for divergence in divergences:
    divergence_label = 'divergence_%f' %divergence
    clade_types.append(divergence_label)
...
def load_ld_map(species_name):
    ...
    ld_directory = '%slinkage_disequilibria_divergence/' % (parse_midas_data.data_directory)
    ...
    for species_name in good_species_list:
        ...
        for clade_type in clade_types:
            ...
            for variant_type in variant_types:
                ...
                ld_dict[(clade_type, variant_type)] = {}

                if len(ld_map)>0:

                    distances, rsquared_numerators, rsquared_denominators, ns, intergene_distances, intergene_rsquared_numerators, intergene_rsquared_denominators, intergene_ns, control_rsquared_numerators, control_rsquared_denominators, control_n, pi = ld_map[(clade_type, variant_type)]
                ...
                distances, rsquared_numerators, rsquared_denominators, ns, intergene_distances, intergene_rsquared_numerators, intergene_rsquared_denominators, intergene_ns, control_rsquared_numerator, control_rsquared_denominator, control_n, pi = ld_map[(clade_type, variant_type)]
                ...
                ld_dict[(clade_type, variant_type)]['distances'] = distances
                ld_dict[(clade_type, variant_type)]['good_distances'] = good_distances
                ld_dict[(clade_type, variant_type)]['early_distances'] = early_distances

                ld_dict[(clade_type, variant_type)]['rsquareds'] = rsquareds
                ld_dict[(clade_type, variant_type)]['lower_rsquareds'] = lower_rsquareds
                ld_dict[(clade_type, variant_type)]['upper_rsquareds'] = upper_rsquareds
                ld_dict[(clade_type, variant_type)]['early_rsquareds'] = early_rsquareds
                ld_dict[(clade_type, variant_type)]['control_rsquared'] = control_rsquared
...
if __name__=='__main__':
    ...
    for species_name in good_species_list:
        ...
        snp_samples_coarse_grained = coarse_grained_samples
        ...
        #clade_types = ['all','largest_clade']
        divergence_sample_dict = {}
        for divergence in divergences:

            sys.stderr.write("Clustering samples with low divergence...\n")
            coarse_grained_idxs_divergence, coarse_grained_cluster_list_divergence = clade_utils.cluster_samples(substitution_rate, min_d=divergence)
            coarse_grained_samples_divergence = snp_samples[coarse_grained_idxs_divergence]

            sys.stderr.write("Divergence = %0.6f: %d samples remaining after clustering!\n" % (divergence, len(coarse_grained_samples_divergence) ))

            clade_idxss_divergence = clade_utils.calculate_clade_idxs_from_clade_sets(coarse_grained_samples_divergence, clade_sets)

            clade_sizes_divergence = numpy.array([clade_idxs.sum() for clade_idxs in clade_idxss_divergence])

            largest_clade_samples_divergence = coarse_grained_samples_divergence[ clade_idxss_divergence[clade_sizes_divergence.argmax()] ]

            largest_clade_set_divergence = set(largest_clade_samples_divergence)


            sys.stderr.write("Top level clades: %d clades, sizes: %s\n" % (len(clade_sets), str(clade_sizes_divergence)))
            sys.stderr.write("Max clade size: %d\n" % len(largest_clade_samples))

            snp_samples_divergence = largest_clade_samples_divergence

            if len(largest_clade_samples_divergence) < min_ld_sample_size:
                sys.stderr.write("Not enough ld samples!\n")
                continue
            else:
                sys.stderr.write("Proceeding with %d coarse-grained samples in largest clade!\n" % len(largest_clade_samples_divergence))
            divergence_label = 'divergence_%f' %divergence
            #clade_types.append(divergence_label)

            divergence_sample_dict[divergence_label] = {}
            divergence_sample_dict[divergence_label]['largest_clade_set_divergence'] = largest_clade_set_divergence
            divergence_sample_dict[divergence_label]['largest_clade_samples_divergence'] = largest_clade_samples_divergence
            ...
            for clade_type in clade_types:
                ...
                for variant_type in variant_types:
                    ...
                    for gene_name in allele_counts_map.keys():
                        ...
                        if 'divergence_' in clade_type:
                            largest_clade_set_divergence = divergence_sample_dict[clade_type]['largest_clade_set_divergence']
                            largest_clade_idxs = numpy.array([sample in largest_clade_set_divergence for sample in snp_samples])
                            allele_counts = allele_counts[:,largest_clade_idxs,:]
                        ...
                        for neighbor_distance_idx in xrange(0,len(neighbor_distances)):
                            ...
                            for neighboring_gene_name in neighboring_genes:
                                ...
                                if (clade_type=='largest_clade') or ('divergence_' in clade_type):
                                    neighboring_allele_counts = neighboring_allele_counts[:,largest_clade_idxs,:]
        ...
        for clade_type in clade_types:
            for variant_type in variant_types:
                ...
                if 'divergence_' in clade_type:
                    desired_samples = divergence_sample_dict[clade_type]['largest_clade_samples_divergence']
