...
def make_plot(variant_type):
    ...
    for column_idx, column in enumerate(nested_species_list):
        ...
        for row_idx, row in enumerate(column):
            ...
            f_mean_slm = prevalence_dict_mapgd[row][clade_type][pi_type][variant_type]['f_mean_slm']
    ...
    # dpi = 600
    fig.savefig("%smean_f_vs_prevalence_%s.png" % (config.analysis_directory, variant_type), format='png', bbox_inches = "tight", pad_inches = 0.4, dpi=600)
    plt.close()
