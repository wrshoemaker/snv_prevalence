...
if __name__=='__main__':
    ...
    for species_name in good_species_list:
        ...
        synonymous_sfs = synonymous_sfs/synonymous_sfs.sum()
        nonsynonymous_sfs = nonsynonymous_sfs/nonsynonymous_sfs.sum()

        mafs = mafs[mafs<maf_cutoff]

        synonymous_sfs = synonymous_sfs[:len(mafs)]
        nonsynonymous_sfs = nonsynonymous_sfs[:len(mafs)]


        record_str = ", ".join([species_name, str(n), ":".join([str(maf) for maf in mafs]), ":".join([str(nonsynonymous_sf) for nonsynonymous_sf in nonsynonymous_sfs]), ":".join([str(synonymous_sf) for synonymous_sf in synonymous_sfs]) ])
