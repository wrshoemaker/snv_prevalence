...
for species_name in species_list:
    ...
    #error_no_zeros_log10 = numpy.log10(error[error>0])
    #error_no_zeros_mapgd_log10_rescaled = (error_no_zeros_log10 - numpy.mean(error_no_zeros_log10)) / numpy.std(error_no_zeros_log10)
    #hist_error, bin_edges_error = numpy.histogram(error_no_zeros_mapgd_log10_rescaled, density=True, bins=20)
    #bins_mean_error = [0.5 * (bin_edges_error[i] + bin_edges_error[i+1]) for i in range(0, len(bin_edges_error)-1 )]
    #bins_mean_error = numpy.asarray(bins_mean_error)
    #hist_error_to_plot = hist_error[hist_error>0]
    #bins_mean_error_to_plot = bins_mean_error[hist_error>0]
    #ax_error_dist.scatter(bins_mean_error_to_plot, hist_error_to_plot, alpha=0.9, s=10, c=species_color_map[species_name])
...
fig.savefig("%sprevalence_summary_%s_%s.png" % (config.analysis_directory, clade_type, variant_type), format='png', bbox_inches = "tight", pad_inches = 0.2, dpi = 600)
plt.close()
