...
def make_plot(clade_type, cov_cutoff):
    ...
    if cov_cutoff == True:
        predicted_prevalence_type = 'predicted_prevalence_slm_minor_cov_threshold'
        observed_prevalence_type = 'observed_prevalence_slm_minor_cov_threshold'
        label_ = 'with_cutoff'
    else:
        predicted_prevalence_type = 'predicted_prevalence_slm'
        observed_prevalence_type = 'observed_prevalence_slm'
        label_ = 'without_cutoff'
    ...
    for column_idx, column in enumerate(nested_species_list):
        ...
        for row_idx, row in enumerate(column):
            ...
            predicted_prevalence = prevalence_dict_mapgd[row][clade_type][pi_type][variant_type][predicted_prevalence_type]
            predicted_prevalence = numpy.asarray(predicted_prevalence)

            observed_prevalence = prevalence_dict_mapgd[row][clade_type][pi_type][variant_type][observed_prevalence_type]
    ...
    # dpi = 600
    fig.savefig("%spredicted_observed_prevalence_mapgd_slm_%s_%s_%s_%s.png" % (config.analysis_directory, clade_type, pi_type, variant_type, label_), format='png', bbox_inches = "tight", pad_inches = 0.4)
    plt.close()
