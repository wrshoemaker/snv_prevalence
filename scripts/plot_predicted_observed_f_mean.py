...
def make_plot(variant_type):
    ...
    for column_idx, column in enumerate(nested_species_list):
        ...
        for row_idx, row in enumerate(column):
            ...
            if (row_idx == 0):
                ax.set_ylabel('Predicted mean SNV frequency', fontsize=16)

            if column_idx == len(nested_species_list)-1:
                ax.set_xlabel('Observed mean SNV frequency', fontsize=16)
    ...
    # dpi = 600
    fig.savefig("%spredicted_observed_f_mean_mapgd_%s.png" % (config.analysis_directory, variant_type), format='png', bbox_inches = "tight", pad_inches = 0.4, dpi = 600)
    plt.close()
