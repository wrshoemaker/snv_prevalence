...
def make_plot(variant_type):
    ...
    for column_idx, column in enumerate(nested_species_list):
        ...
        for row_idx, row in enumerate(column):
            ...
            predicted_f_mean = prevalence_dict_mapgd[row][clade_type][pi_type][variant_type]['predicted_f_var_mapgd']
            predicted_f_mean = numpy.asarray(predicted_f_mean)

            observed_f_mean = prevalence_dict_mapgd[row][clade_type][pi_type][variant_type]['f_var_mapgd']
            ...
            if (row_idx == 0):
                ax.set_ylabel('Predicted variance of SNV frequencies', fontsize=13)

            if column_idx == len(nested_species_list)-1:
                ax.set_xlabel('Observed variance of SNV frequencies', fontsize=13)

            if column_idx == len(nested_species_list)-2:
                if row_idx > len(nested_species_list[-1])-1:
                    ax.set_xlabel('Observed SNV variance', fontsize=13)


    fig.tight_layout()
    fig.subplots_adjust(hspace=0.2)
    # dpi = 600
    fig.savefig("%spredicted_observed_f_var_mapgd_%s.png" % (config.analysis_directory, variant_type), format='png', bbox_inches = "tight", pad_inches = 0.4, dpi = 600)
    plt.close()
