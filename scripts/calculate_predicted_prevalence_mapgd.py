...
def make_pi_and_parameter_dict(species_to_run='all'):
    ...
    for species_name in species_to_run:
        ...
        for line in snp_file:
            ...
            #samples_pass_coverage = samples[depths>D_min]
...
def predict_prevalence(f_max, pi_filter, depths, min_minor_allele_cov=1):

    prob_zero = (1 +  f_max * depths) ** (-1*pi_filter)

    if min_minor_allele_cov > 1:
        # range() excludes upper bound
        cumulative_sum = 0
        for n_i in range(0, min_minor_allele_cov):
            cumulative_sum += (scipy.special.gamma(pi_filter+n_i) / numpy.math.factorial(n_i)) * ((depths / (depths + (f_max**-1))) ** n_i) / scipy.special.gamma(pi_filter)
...
def predict_prevalence_slm(freqs, depths, f_mean=None, beta=None, min_minor_allele_cov=1):
    # min_cov=1,
...
def joint_gamma_parameter_simulation(x_mean_observed, beta_observed, D, iter = 1000, species_name=None):
    ...
    x_mean_all_log10 = numpy.random.uniform(low=x_mean_log10_min, high=-0.3, size=iter)
    beta_all_log10 = numpy.random.uniform(low=-3, high=2, size=iter)
    ...
    for species_name in species_to_run:
        ...
        for clade_type in clade_types:
            ...
            things_to_measure = ['f_max_all', 'observed_prevalence_all', 'f_mean_all', 'predicted_prevalence_all', \
                                ...
                                'f_mean_slm', 'beta_slm', 'f_mean_slm_best', 'beta_slm_best', 'min_distance_slm', 'predicted_prevalence_slm_best', 'observed_prevalence_slm_best']
            ...
            sys.stderr.write("%s\n" % clade_type)
            sys.stderr.write("Subsetting parameter dict...\n")
            ...
            for key, value in parameter_dict_clade.items():
                ...
                for pi_type in ['pi_exclude_boundary', 'pi_include_boundary']:
                    ...
                    predicted_prevalence = predict_prevalence(f_max_i, pi_filter, depths_i_filter, min_minor_allele_cov=10)
                    ...
                    #print(f_mean_best, beta_best, min_distance_slm )
                    ...
                    # run folded analyses
                    #f_i_filter_folded = numpy.asarray([min([f_, 1-f_]) for f_ in f_i_filter])
                    #f_i_filter_folded = numpy.asarray([max([f_, 1-f_]) for f_ in f_i_filter])
                    #
                    #f_i_filter_folded_no_zeros = f_i_filter_folded[f_i_filter_folded>0]
                    #f_i_filter_folded_no_zeros = f_i_filter_folded[(f_i_filter_folded>0) & (f_i_filter_folded<1)]
                    #f_max_i_folded = max(f_i_filter_folded)
                    #predicted_prevalence_folded = 1 - numpy.mean( (1 +  f_max_i_folded * depths_i_filter) ** (-1*pi_filter) )
                    ...
                    #prevalence_dict_mapgd[pi_type][location_aa_i]['n_non_zero_frequencies'].append(len(f_no_zeros_i))
                    ...
                    prevalence_dict_mapgd[pi_type][location_aa_i]['predicted_prevalence_slm_best'].append(predicted_prevalence_slm_best)
                    prevalence_dict_mapgd[pi_type][location_aa_i]['observed_prevalence_slm_best'].append(observed_prevalence_slm_best)
                    ...
                    # folded
                    #prevalence_dict_mapgd[pi_type][location_aa_i]['f_max_mapgd_folded'].append(f_max_i_folded)
                    #prevalence_dict_mapgd[pi_type][location_aa_i]['predicted_prevalence_mapgd_folded'].append(predicted_prevalence_folded)
                    #prevalence_dict_mapgd[pi_type][location_aa_i]['f_mean_mapgd_folded'].append(f_mean_i_folded)
                    #prevalence_dict_mapgd[pi_type][location_aa_i]['predicted_f_mean_mapgd_folded'].append(predicted_f_mean_folded)
                    #prevalence_dict_mapgd[pi_type][location_aa_i]['f_var_mapgd_folded'].append(f_var_i_folded)
                    #prevalence_dict_mapgd[pi_type][location_aa_i]['predicted_f_var_mapgd_folded'].append(predicted_f_var_folded)
                    #prevalence_dict_mapgd[pi_type][location_aa_i]['f_no_zeros_mapgd_folded'].extend(f_i_filter_folded_no_zeros.tolist())
                    #prevalence_dict_mapgd[pi_type][location_aa_i]['predicted_prevalence_mapgd_slm_folded'].append(predicted_prevalence_slm_folded)
                    #prevalence_dict_mapgd[pi_type][location_aa_i]['observed_prevalence_mapgd_slm_folded'].append(observed_prevalence_slm_folded)
...
def load_predicted_prevalence_dict_all():
...
if __name__=='__main__':
    ...
    #parser.add_argument("--species", help="Name of specific species to run code on", default='none')
    parser.add_argument("--make_alleles_dict", help="Get alleleic states for MAPGD", default='none')
