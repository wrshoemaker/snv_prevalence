...
def permutation_ks_test(array_1, array_2, n_iter=10000):
    n_1 = len(array_1)
    d, p = ks_2samp(array_1, array_2)

    array_concat = numpy.concatenate([array_1, array_2])
    null_d_all = []
    for i in range(n_iter):
        array_null = numpy.random.permutation(array_concat)
        array_null_1 = array_null[:n_1]
        array_null_2 = array_null[n_1:]
        d_null, p_null = ks_2samp(array_null_1, array_null_2)

        null_d_all.append(d_null)

    null_d_all = numpy.asarray(null_d_all)
    p = sum(null_d_all>d) / n_iter
