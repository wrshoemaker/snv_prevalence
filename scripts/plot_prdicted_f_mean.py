...
for column_idx, column in enumerate(nested_species_list):
    ...
    for row_idx, row in enumerate(column):
            ...
            predicted_f_mean = prevalence_dict[row]['all']['4D']['predicted_f_mean_to_plot']
            predicted_f_mean = numpy.asarray(predicted_f_mean)

            observed_f_mean = prevalence_dict[row]['all']['4D']['f_mean_to_plot']
            observed_f_mean = numpy.asarray(observed_f_mean)

            all_ = numpy.concatenate([predicted_f_mean,observed_f_mean])


            # Calculate the point density
            xy = numpy.vstack([observed_f_mean, predicted_f_mean])
            z = gaussian_kde(xy)(xy)

            # Sort the points by density, so that the densest points are plotted last
            idx = z.argsort()
            x, y, z = observed_f_mean[idx], predicted_f_mean[idx], z[idx]
