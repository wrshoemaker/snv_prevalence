...
for column_idx, column in enumerate(nested_species_list):
    ...
    for row_idx, row in enumerate(column):
            ...
            predicted_prevalence = prevalence_dict[row][clade_type]['4D']['predicted_prevalence_trunc']
            predicted_prevalence = numpy.asarray(predicted_prevalence)

            observed_prevalence = prevalence_dict[row][clade_type]['4D']['observed_prevalence_trunc']
...
# dpi = 600
fig.savefig("%spredicted_observed_prevalence_trunc_%s.png" % (config.analysis_directory, clade_type), format='png', bbox_inches = "tight", pad_inches = 0.4)
plt.close()
